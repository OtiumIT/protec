...
const userService = new user_service_1.UserService(userRepo, subscriptionService);
...
userRoutes.use('/*', tenant_middleware_1.tenantMiddleware);
...
adminRoutes.use('*', auth_middleware_1.authMiddleware);
/**
 ...
 */
adminRoutes.post('/admin', (0, zod_validator_1.zValidator)('json', core_1.CreateUserSchema), async (c) => {
    try {
        const currentUser = c.get('user');
        if (currentUser.role !== 'super_admin') {
            return c.json({ error: { message: 'Forbidden', code: 'FORBIDDEN' } }, 403);
        ...
        return c.json({ data: { user: toUserResponse(user) } }, 201);
    }
    catch (error) {
        return (0, error_handler_1.errorHandler)(error, c);
    }
 ...
 */
adminRoutes.post('/admin/super-admin', (0, zod_validator_1.zValidator)('json', core_1.CreateUserSchema), async (c) => {
    try {
        const currentUser = c.get('user');
        if (currentUser.role !== 'super_admin') {
            return c.json({ error: { message: 'Forbidden', code: 'FORBIDDEN' } }, 403);
 ...
 * Criar usuário
 */
userRoutes.post('/', (0, zod_validator_1.zValidator)('json', core_1.CreateUserSchema, (result, c) => {
    if (!result.success) {
        const message = formatCreateUserValidationMessage(result.error);
        return c.json({ error: { message, code: 'VALIDATION_ERROR', details: result.error.errors } }, 400);
    }
    return;
}), async (c) => {
    try {
 ...
 * Atualizar usuário
 */
userRoutes.put('/:id', (0, zod_validator_1.zValidator)('json', core_1.UpdateUserSchema), async (c) => {
    try {
        const tenantId = c.get('companyId');
        if (!tenantId) {
            return c.json({ error: { message: 'Tenant required', code: 'TENANT_REQUIRED' } }, 400);
