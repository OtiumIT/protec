...
exports.impostoIRPFMensal = impostoIRPFMensal;
exports.calcularPF = calcularPF;
exports.calcularPJ = calcularPJ;
exports.calcularReforma2027 = calcularReforma2027;
exports.calcularBreakEven = calcularBreakEven;
...
function calcularPF(aggregated, aliquotaEfetivaDirpf) {
    ...
    const aliquotaEfetiva = aliquotaEfetivaDirpf !== undefined && aliquotaEfetivaDirpf >= 0
...
function adicionalIRPJ(baseCalculoTrimestre) {
    if (baseCalculoTrimestre <= LIMITE_LUCRO_PRESUMIDO_ADICIONAL)
        return 0;
...
function calcularPJ(aggregated, elegivelPresuncao16) {
    ...
    const impostoTotal = trimestres.reduce((s, x) => s + x.irpj + x.irpj_adicional + x.irpj_postergado + x.csll + x.pis + x.cofins, 0);
    const aliquotaEfetiva = receita_total > 0 ? (impostoTotal / receita_total) * 100 : 0;
    return {
        receita_bruta_total: round2(receita_total),
        base_presumida_irpj: round2(trimestres.reduce((s, x) => s + x.base_irpj, 0)),
        base_presumida_csll: round2(trimestres.reduce((s, x) => s + x.base_csll, 0)),
...
function calcularReforma2027(aggregated, aliquotaIbsCbsOverride, redutorLocacaoPct, opcoes) {
    ...
    const aliquotaNominal = aliquotaIbsCbsOverride ??
    ...
    const usarRedutorDiferenciado = opcoes?.usar_redutor_diferenciado_short === true &&
...
function calcularBreakEven(cargaPFPercentual, cargaPJPercentual) {
    if (cargaPJPercentual >= cargaPFPercentual)
        return null;
