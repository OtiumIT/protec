...
const schema_manager_1 = require("../../db/schema-manager");
...
class CompanyService {
    ...
    async create(data) {
        ...
        try {
            ...
            if (data.domain) {
                const existingDomain = await client.query('SELECT id FROM companies WHERE domain = $1 FOR UPDATE', [data.domain]);
                if (existingDomain.rows.length > 0) {
                    await client.query('ROLLBACK');
                    throw new error_handler_1.AppError('Domain already exists', 'DOMAIN_ALREADY_EXISTS', 409);
                }
            }
            if (data.cnpj) {
                const existingCnpj = await client.query('SELECT id FROM companies WHERE cnpj = $1 FOR UPDATE', [data.cnpj]);
                ...
                try {
                    ...
                    const existingByName = await client.query('SELECT id FROM companies WHERE name = $1 AND (email = $2 OR ($2 IS NULL AND email IS NULL)) AND (cnpj IS NULL OR cnpj = \'\') FOR UPDATE', [data.name, data.email || null]);
                    if (existingByName.rows.length > 0) {
                        await client.query('ROLLBACK');
                        throw new error_handler_1.AppError('Company with same name and email already exists', 'COMPANY_ALREADY_EXISTS', 409);
                    }
                }
                finally {
            ...
            const schemaExists = await client.query(`SELECT schema_name FROM information_schema.schemata WHERE schema_name = $1`, [schemaName]);
            if (schemaExists.rows.length === 0) {
                // Criar schema do tenant (usando mesmo client da transação)
                await (0, schema_manager_1.createTenantSchema)(company.id, client);
                // Aplicar migrations de tenant no novo schema (usando mesmo client da transação)
                await (0, schema_manager_1.applyTenantMigrations)(company.id, client);
            }
            else {
