...
exports.fiscalFileRoutes = void 0;
const hono_1 = require("hono");
const zod_validator_1 = require("@hono/zod-validator");
const fiscal_file_service_1 = require("./fiscal-file.service");
const fiscal_file_repository_1 = require("./fiscal-file.repository");
...
const fiscalFileRoutes = new hono_1.Hono();
exports.fiscalFileRoutes = fiscalFileRoutes;
// Aplicar middlewares globais
fiscalFileRoutes.use('/*', tenant_middleware_1.tenantMiddleware);
fiscalFileRoutes.use('/*', auth_middleware_1.authMiddleware);
fiscalFileRoutes.use('/*', (0, module_middleware_1.requireModule)('FISCAL_FILES'));
// Instanciar services
...
const fiscalFileService = new fiscal_file_service_1.FiscalFileService(fiscalFileRepo, clientRepo);
/**
...
fiscalFileRoutes.post('/upload', async (c) => {
    try {
        ...
        // Validar schema
        const validation = core_1.UploadFiscalFileSchema.safeParse({
            client_id: clientId,
            competence,
            file_type: fileType,
        });
        ...
        const fiscalFile = await fiscalFileService.upload(companyId, validation.data.client_id, validation.data.competence, validation.data.file_type, buffer, file.name, file.type, userId);
        return c.json({
            data: {
                fiscal_file: {
                    id: fiscalFile.id,
 ...
 */
fiscalFileRoutes.get('/', (0, zod_validator_1.zValidator)('query', core_1.ListFiscalFilesQuerySchema), async (c) => {
    try {
 ...
 */
fiscalFileRoutes.get('/:id', (0, zod_validator_1.zValidator)('param', core_1.FiscalFileIdParamSchema), async (c) => {
    try {
 ...
 */
fiscalFileRoutes.get('/:id/download', (0, zod_validator_1.zValidator)('param', core_1.FiscalFileIdParamSchema), (0, zod_validator_1.zValidator)('query', core_1.DownloadFiscalFileQuerySchema), async (c) => {
    try {
        const { id } = c.req.valid('param');
 ...
 */
fiscalFileRoutes.put('/:id/status', (0, zod_validator_1.zValidator)('param', core_1.FiscalFileIdParamSchema), (0, zod_validator_1.zValidator)('json', core_1.UpdateFiscalFileStatusSchema), async (c) => {
    try {
        const { id } = c.req.valid('param');
 ...
 */
fiscalFileRoutes.delete('/:id', (0, zod_validator_1.zValidator)('param', core_1.FiscalFileIdParamSchema), async (c) => {
    try {
        const { id } = c.req.valid('param');
        const companyId = c.get('companyId');
        if (!companyId) {
 ...
 */
fiscalFileRoutes.get('/client/:client_id', (0, zod_validator_1.zValidator)('param', core_1.ClientIdParamSchema), async (c) => {
    try {
