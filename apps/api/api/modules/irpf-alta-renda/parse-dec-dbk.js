...
exports.DEC_DBK_PARSER_VERSION = void 0;
exports.parseDecDbk = parseDecDbk;
...
function extractAnoFromFilename(name) {
    const match = name.match(/_(20\d{2})_IRPF/i) ||
...
exports.DEC_DBK_PARSER_VERSION = 3;
const CODIGO_ISENTO_DESCRICAO = {
...
function parseFixedWidth(lines, filename) {
    ...
    const dependentes = [];
    const bensDireitos = [];
    const codigosValor = {}; // codigo -> valor
    const avisos = [];
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        if (tipo === '21') {
            ...
            if (base20 > 0 && baseCalculoIr === 0)
                baseCalculoIr = base20;
            if (imposto20 >= 0 && impostoDevido === 0)
                impostoDevido = imposto20;
        }
        else if (tipo === '19') {
            ...
            if (v5 > 0 && v5 < 1e9)
                impostoPagoCarneLeao = v5;
            if (v11 > 0 && v11 < 1e9)
                impostoPagoRetencao = v11;
        }
        else if (tipo === '25') {
    ...
    if (!nome && cpf)
        nome = 'Contribuinte (importado)';
    if (!nome && !cpf)
        nome = 'Contribuinte (verifique os dados)';
    if (!cpf)
        cpf = '00000000000';
    return buildResult(ano, nome, cpf, totalRendPj, totalRendPf, itensPj, itensPf, itensIsentos09, itensIsentos13, itensIsentosOutros, baseCalculoIr, impostoDevido, impostoPagoRetencao, impostoPagoCarneLeao, tot09, tot13, dependentes, bensDireitos, {
...
function buildResult(ano, nome, cpf, totalRendPj, totalRendPf, itensPj, itensPf, itensIsentos09, itensIsentos13, itensIsentosOutros, baseCalculoIr, impostoDevido, impostoPagoRetencao, impostoPagoCarneLeao, tot09 = 0, tot13 = 0, dependentes = [], bensDireitos = [], diagnostico) {
    ...
    const rendimentos_isentos_dividendos = [
        ...itensIsentos09.map((i) => ({ ...i, valor: round2(i.valor), codigo: '09' })),
        ...itensIsentos13.map((i) => ({ ...i, valor: round2(i.valor), codigo: '13' })),
    ];
    const classificacaoIsentos = (0, calculations_1.classificarIsentosArt16A)([
    ...
    const declaracaoValidada = core_1.DeclaracaoIrpfCompletaSchema.parse(declaracao_completa);
    const dadosValidados = core_1.DadosIrpfAltaRendaSchema.parse(dadosParsed);
    return {
        ano,
        dados: dadosValidados,
        declaracao_completa: declaracaoValidada,
        parser_version: exports.DEC_DBK_PARSER_VERSION,
        diagnostico,
    };
}
/**
...
function parseDecDbk(buffer, filename = '') {
    ...
    throw new Error('Formato não reconhecido. Use arquivo .dec ou .dbk do Programa IRPF ou e-CAC.');
}
function parsePipeDelimited(lines, filename) {
    ...
    for (const fields of records) {
        ...
        if (tipo.startsWith('DECPF') || tipo.startsWith('RESPO') || tipo === '00' || tipo === '01') {
            for (let i = 0; i < fields.length; i++) {
                ...
                if (cpfCandidate.length === CPF_LENGTH && !cpf)
                    cpf = cpfCandidate;
        ...
        if (tipo.startsWith('RTRT') || tipo === '10' || tipo === '11') {
            ...
            for (let i = 1; i < fields.length; i++) {
                ...
                if (digits.length === 14)
                    cnpj = digits;
                if (typeof fields[i] === 'string' && fields[i].length > 2 && !/^\d+$/.test(fields[i]))
                    desc = fields[i];
            }
            if (valor > 0) {
        ...
        if (/09|090/.test(codigoReceita) || tipo.includes('09')) {
            ...
            if (v > 0)
                itensIsentos09.push({ nome_fonte: fields[1] ?? 'Dividendos', valor: v });
        }
        if (/13|130/.test(codigoReceita) || tipo.includes('13')) {
            const v = parseValorMonetario(valorField);
            if (v > 0)
                itensIsentos13.push({ nome_fonte: fields[1] ?? 'Sócio Simples', valor: v });
        }
    ...
    return buildResult(ano, nome, cpf, totalRendPj, totalRendPf, itensPj, itensPf, itensIsentos09, itensIsentos13, itensIsentosOutros, baseCalculoIr, impostoDevido, impostoPagoRetencao, 0, // impostoPagoCarneLeao (não extraído no layout pipe)
    0, 0, [], // dependentes (layout pipe não parseia tipo 25)
