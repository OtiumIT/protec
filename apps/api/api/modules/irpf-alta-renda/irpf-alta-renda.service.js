...
exports.IrpfAltaRendaService = void 0;
...
function buildMemoriaCalculo(input, bcc, resultado) {
    ...
    if (dividendos.length > 0) {
        premissasAplicadas.push('Risco de retenção mensal foi avaliado por aproximação (valor anual por fonte dividido por 12).');
    }
    const itensLei7713 = input.dados.rendimentos_tributados_exclusivamente_lei_7713 ?? [];
    if (itensLei7713.some((i) => (i.irrf ?? 0) <= 0)) {
        premissasAplicadas.push('Itens da Lei 7.713 sem IRRF informado usam alíquota estimada para simulação de compensação.');
    }
    return {
        ...
        fonte_normativa: calculations_1.CONFIG_LEI_15270_2025.fonte_normativa,
        observacao_progressiva: calculations_1.CONFIG_LEI_15270_2025.observacao_progressiva,
        premissas_aplicadas: premissasAplicadas,
    };
}
class IrpfAltaRendaService {
    repo;
    companyRepo;
    ...
    async simulate(input) {
        ...
        if ((input.dados.optou_ajuste_anual_lei_7713 ?? false) && rendimentosLei7713.length > 0) {
            throw new error_handler_1.AppError('Rendimentos da Lei 7.713 (art. 12-A) só podem ser tratados como exclusão quando não há opção pelo ajuste anual.', 'LEI_7713_AJUSTE_ANUAL_INCOMPATIVEL', 422);
        }
        const lucrosExcl = input.dados.lucros_aprovados_ate_31dez2025 ?? 0;
        const ganhoCapitalExcl = input.dados.ganho_capital_excluido ?? 0;
        const fiisExcl = input.dados.rendimentos_fiis_excluidos ?? 0;
        ...
        const bcc = (0, calculations_1.calcularBCC)(input.dados.rendimentos_tributaveis, input.dados.rendimentos_isentos_dividendos, lucrosExcl, ganhoCapitalExcl, fiisExcl, outrosIsentosQueEntramBase, outrosExclArt16A);
        const resultado = (0, calculations_1.aplicarFaixas)(bcc);
        ...
        const risco = (0, calculations_1.avaliarRiscoRetencao)(input.dados.rendimentos_isentos_dividendos);
        const memoria_calculo = buildMemoriaCalculo(input, bcc, resultado);
        const ANO_VIGENCIA_IRPFM = 2027;
        const avisoAnoForaVigencia = input.ano < ANO_VIGENCIA_IRPFM
        ...
        const composicaoRenda = (0, calculations_1.comporRendaParaDashboard)(input.dados);
        const impactoIncrementalBase = (0, calculations_1.calcularImpactoIncrementalBase)(input.dados);
        const memoriaLegalExclusoes = (0, calculations_1.construirMemoriaLegalExclusoes)(input.dados);
        const otimizacao = (0, calculations_1.simularOtimizacaoIsentoVsTributado)(input.dados, bcc, impostoComplementar, deducoesTotal);
        const sugestoes_planejamento = (0, calculations_1.gerarSugestoesPlanejamento)(input.dados, {
        ...
        if (otimizacao) {
            ...
            if (otimizacao.ganho_liquido_estimado > 0) {
                sugestoes_planejamento.unshift(`Otimizacao de carteira: migracao simulada de ${otimizacao.valor_migrado.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })} para ativo tributado com IRRF compensavel pode gerar ganho liquido estimado de ${otimizacao.ganho_liquido_estimado.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}.`);
            }
        }
        const valorHipotetico = input.dados.valor_hipotetico_comparativo_pf_pj;
        const somaLei7713 = (input.dados.rendimentos_tributados_exclusivamente_lei_7713 ?? []).reduce((s, i) => s + (i.valor_bruto ?? 0), 0);
        ...
        const valorAplicacaoRef = Math.max(1000, (valorHipotetico != null && valorHipotetico > 0 ? valorHipotetico : null) ??
        ...
        const comparativoPfPj = (0, calculations_1.compararEficienciaPfPj)(valorAplicacaoRef, input.dados, {
            base_calculo_combinada: resultado.base_calculo_combinada,
            imposto_estimado: impostoComplementar,
        ...
        if (comparativoPfPj && comparativoPfPj.diferenca_percentual_pj_mais_caro > 0) {
            sugestoes_planejamento.push(`Investir via PJ: para aplicacao de ${comparativoPfPj.rendimento_bruto.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}, a carga na PJ (Lucro Presumido) e ${comparativoPfPj.diferenca_percentual_pj_mais_caro.toFixed(1)}% maior em relacao ao cenario PF.`);
        }
        if (comparativoPfPj) {
            ...
            memoria_calculo.irrf_compensavel_pj = {
                ...
                observacao: 'IRRF retido na fonte sobre receitas financeiras na PJ (CDB 15-22,5%, JCP 15%, FII 20%). Pode ser compensado com IRPJ devido. Valores por trimestre assumem distribuição uniforme.',
            };
        }
        return {
            ano: input.ano,
    ...
    async simulateAndSave(input, userId) {
        if (input.company_id) {
            ...
            if (!company) {
                throw new error_handler_1.AppError('Empresa não encontrada', 'COMPANY_NOT_FOUND', 404);
            }
        }
    ...
    async getById(id) {
        ...
        if (!record) {
            throw new error_handler_1.AppError('Simulação IRPF Alta Renda não encontrada', 'IRPF_ALTA_RENDA_NOT_FOUND', 404);
        }
...
exports.IrpfAltaRendaService = IrpfAltaRendaService;
