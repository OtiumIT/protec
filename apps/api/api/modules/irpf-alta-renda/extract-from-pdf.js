...
exports.extractIrpfFromPdf = extractIrpfFromPdf;
const openai_1 = __importDefault(require("openai"));
const core_1 = require("@shared/core");
const zod_1 = require("zod");
const calculations_1 = require("./calculations");
// ── Formatos de fallback (LLM às vezes retorna estrutura diferente) ─────────────
const OldFormatRendPJ = zod_1.z.object({ fonte: zod_1.z.string().optional(), cnpj: zod_1.z.string().optional(), valor: zod_1.z.number().nonnegative().default(0) });
const OldFormatRendPF = zod_1.z.object({ mes: zod_1.z.string().optional(), valor: zod_1.z.number().nonnegative().default(0) });
const OldFormatRendIsento = zod_1.z.object({ nome_fonte: zod_1.z.string().optional(), cnpj_fonte: zod_1.z.string().optional(), valor: zod_1.z.number().nonnegative().default(0) });
const OldFormatSchema = zod_1.z.object({
    ano: zod_1.z.number().int().min(2020).max(2035).default(new Date().getFullYear()),
    contribuinte: zod_1.z.object({ nome: zod_1.z.string().default(''), cpf: zod_1.z.string().default('') }),
    base_calculo_alta_renda: zod_1.z.object({
        tributaveis_pj: zod_1.z.array(OldFormatRendPJ).default([]),
        tributaveis_pf_alugueis: zod_1.z.array(OldFormatRendPF).default([]),
        isentos_lucros_dividendos: zod_1.z.array(OldFormatRendIsento).default([]),
        isentos_simples_nacional: zod_1.z.array(OldFormatRendIsento).default([]),
    }),
    outros_rendimentos: zod_1.z.object({
        aplicacoes_financeiras_exclusiva: zod_1.z.number().nonnegative().default(0),
        juros_capital_proprio: zod_1.z.number().nonnegative().default(0),
        poupanca_lci_lca: zod_1.z.number().nonnegative().default(0),
    }).optional().default({}),
    patrimonio_imobiliario: zod_1.z.array(zod_1.z.object({ descricao: zod_1.z.string().optional(), valor_atual: zod_1.z.number().nonnegative().default(0) })).default([]),
}).passthrough();
...
function previewForLog(value, maxLen = 2000) {
...
async function extractSinglePassFromText(openai, text) {
    ...
    const completion = await openai.chat.completions.create({
        ...
        messages: [
            ...
            {
                role: 'user',
                content: `Analise o conteúdo integral da Declaração de IRPF abaixo e extraia todos os campos no formato JSON canônico.\n` +
...
function hasAnyMoneyInSection(section) {
    if (!section)
        return false;
    if (hasMeaningfulMoney(section.total))
        return true;
    for (const item of section.itens ?? []) {
        if (!item || typeof item !== 'object')
            continue;
        for (const v of Object.values(item)) {
            if (hasMeaningfulMoney(v))
                return true;
        }
    }
    return false;
...
function evaluateExtractionQuality(d) {
    ...
    const checks = [
        {
            nome: 'identificacao',
            ok: String(identificacao.nome ?? '').trim().length > 2 ||
                String(identificacao.cpf ?? '').replace(/\D/g, '').length >= 11,
        },
        {
            nome: 'tributaveis',
            ok: hasAnyMoneyInSection(pj) ||
    ...
    const itemSignals = (pj?.itens?.length ?? 0) +
        (pf?.itens?.length ?? 0) +
...
async function extractIrpfFromPdf(pdfBuffer) {
    ...
    let validatedFull = core_1.DeclaracaoIrpfCompletaSchema.safeParse(normalizedBase);
    if (!validatedFull.success) {
        const retryNormalized = collapseUnexpectedObjectArrays(normalizedBase);
        validatedFull = core_1.DeclaracaoIrpfCompletaSchema.safeParse(retryNormalized);
    }
    const warnings = [];
    if (validatedFull.success) {
        ...
        if (quality.completude === 'baixa')
            completude = 'baixa';
        if (quality.completude === 'media' && completude === 'alta')
            completude = 'media';
...
function normalizePagamentoItem(item) {
    if (item && typeof item === 'object' && !Array.isArray(item))
        return item;
    if (typeof item === 'string' && item.trim())
        return { tipo: item.trim(), valor: 0 };
    const value = coerceMoneyValue(item);
    if (value != null)
        return { tipo: undefined, valor: value };
    return null;
}
function normalizeDoacaoItem(item) {
    if (item && typeof item === 'object' && !Array.isArray(item))
        return item;
    if (typeof item === 'string' && item.trim())
        return { descricao: item.trim(), valor: 0 };
    const value = coerceMoneyValue(item);
    if (value != null)
        return { descricao: undefined, valor: value };
    return null;
}
function normalizeStructuredArrayField(value, kind) {
    ...
    const normalized = raw.map((item) => {
        if (kind === 'dependentes')
            return normalizeDependenteItem(item);
        if (kind === 'pagamentos')
            return normalizePagamentoItem(item);
...
function extractPtBrNumberFromText(raw) {
...
function coerceNumericFields(value, key) {
    ...
    if (typeof value === 'string' && key && NUMERIC_KEYS.has(key) && !NON_NUMERIC_KEYS.has(key)) {
        ...
        if (parsed == null && MONEY_KEYS.has(key))
            return 0;
        if (parsed == null && YEAR_KEYS.has(key))
...
function mapDeclaracaoCompletaToDados(d) {
    ...
    const tributaveis_pj = (rtPj.itens ?? []).map((i) => ({ fonte: i.nome_fonte ?? i.fonte ?? '', cnpj: i.cnpj, valor: round2(i.valor ?? 0) }));
    let tributaveis_pf_alugueis = (rtPf.itens ?? []).map((i) => ({ mes: i.mes ?? '', valor: round2(i.valor ?? 0) }));
    ...
    const classificacaoDeterministica = (0, calculations_1.classificarIsentosArt16A)(isentos.map((i) => ({
    ...
    const poupanca = (d.rendimentos_isentos_nao_tributaveis?.itens ?? [])
        .filter((i) => /poupanca|lci|lca|cra|cri/i.test(String(i.descricao ?? i.codigo ?? '')))
    ...
    const patrimonio_imobiliario = bens
        .filter((i) => ['01', '11', '12'].includes(String(i.codigo ?? '')))
        .map((i) => ({ descricao: i.descricao ?? '', valor_atual: round2(i.valor_atual ?? 0) }));
    const resumo = d.resumo ?? {};
    ...
    const impostoAplicacoes = round2(excl
        .filter((i) => String(i.codigo ?? '').includes('06'))
        .reduce((s, i) => s + (i.irrf ?? 0), 0));
    ...
    const fiisFallback = fiisExcl === 0
        ? round2((d.rendimentos_isentos_nao_tributaveis?.itens ?? []).filter((i) => /fii|fundo imobili|fundo de investimento imobili/i.test(String(i.descricao ?? i.codigo ?? ''))).reduce((s, i) => s + (i.valor ?? 0), 0))
        : fiisExcl;
    const outrosExclFallback = outrosExclLei > 0 ? outrosExclLei : (0, calculations_1.identificarOutrosExcluidosArt16A)(isentos);
    return {
