...
const debugRoutes = new hono_1.Hono();
exports.debugRoutes = debugRoutes;
/**
...
debugRoutes.get('/modules-db', async (c) => {
    try {
        const [modulesRes, companiesRes, tenantModulesRes, simuladorRes] = await Promise.all([
            (0, client_1.query)('SELECT id, name, key, description FROM modules ORDER BY name'),
            (0, client_1.query)('SELECT id, name, domain FROM companies ORDER BY name'),
            (0, client_1.query)(`SELECT tm.tenant_id, tm.module_id, tm.enabled_until, m.key AS module_key, m.name AS module_name
         FROM tenant_modules tm
         JOIN modules m ON m.id = tm.module_id
         ORDER BY tm.tenant_id, m.key`),
            (0, client_1.query)(`SELECT tm.tenant_id, c.name AS company_name, tm.enabled_until
         FROM tenant_modules tm
         JOIN modules m ON m.id = tm.module_id
