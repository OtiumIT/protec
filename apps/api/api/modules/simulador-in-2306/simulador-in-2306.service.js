...
exports.SimuladorIN2306Service = void 0;
const error_handler_1 = require("../../shared/utils/error-handler");
const calculations_1 = require("./calculations");
class SimuladorIN2306Service {
    repo;
    clientRepo;
    ...
    async simulate(input, userId) {
        ...
        if (input.save_simulation && !input.client_id) {
            throw new error_handler_1.AppError('client_id é obrigatório ao salvar a simulação', 'CLIENT_REQUIRED', 400);
        }
    ...
    async getById(id) {
        ...
        if (!simulation) {
            throw new error_handler_1.AppError('Simulação não encontrada', 'SIMULATION_NOT_FOUND', 404);
        }
        return simulation;
    }
    async list(options) {
    ...
    async simulateTributario(input, userId) {
        ...
        const tri2025 = (0, calculations_1.calcularCenario2025)(input.trimestres, ded, ret, false);
        const tri2026 = (0, calculations_1.calcularAno2026)(input.trimestres, ded, ret, false);
        const triEquip = (0, calculations_1.calcularCenario2025)(input.trimestres, ded, ret, true);
        const toCenarioAnual = (trimestres) => {
            const totais = (0, calculations_1.agregarAnual)(trimestres);
            return {
        ...
        for (let t = 0; t < 4; t++) {
            ...
            if (!receitas)
                continue;
            const detalhe = (0, calculations_1.detalheProporcaoTrimestre)(receitas, false, t + 1, input.ano);
            if (detalhe)
                proporcaoTrimestres.push(detalhe);
        }
        const result = {
            ano: input.ano,
            cenario_2025,
...
exports.SimuladorIN2306Service = SimuladorIN2306Service;
