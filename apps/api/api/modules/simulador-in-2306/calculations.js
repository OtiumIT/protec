...
exports.receitaBrutaTrimestre = receitaBrutaTrimestre;
exports.detalheProporcaoTrimestre = detalheProporcaoTrimestre;
exports.calcularTrimestre2025 = calcularTrimestre2025;
exports.calcularAno2026 = calcularAno2026;
exports.calcularCenario2025 = calcularCenario2025;
exports.agregarAnual = agregarAnual;
...
function receitaBrutaTrimestre(r) {
    return ((r.produtos_mercadorias ?? 0) +
        (r.servicos ?? 0) +
...
function detalheProporcaoTrimestre(r, equiparacaoHospitalar, numTrimestre, ano) {
    const total = receitaBrutaTrimestre(r);
    if (total <= 0 || total <= LIMITE_TRIMESTRAL)
        return null;
    const aplicarAcrescimoIrpj = ano >= 2026;
    ...
    for (const key of keys) {
        ...
        const formulaResumida = excedente > 0
...
function adicionalIRPJ(baseCalculoIrpjTrimestre) {
    if (baseCalculoIrpjTrimestre <= LIMITE_LUCRO_PRESUMIDO_ADICIONAL)
        return 0;
...
function calcularAno2026(trimestres, deducoesTrimestrais, retencoesTrimestrais, equiparacao) {
    ...
    const valoresComAcrescimo = resultados.slice(0, 3).reduce((acc, x) => ({
        irpj: acc.irpj + x.irpj + (x.irpj_adicional ?? 0),
        csll: acc.csll + x.csll,
    }), { irpj: 0, csll: 0 });
    const valoresSemAcrescimoT1T3 = [0, 1, 2].reduce((acc, i) => {
...
function calcularCenario2025(trimestres, deducoesTrimestrais, retencoesTrimestrais, equiparacao) {
    ...
    return trimestres.map((r, i) => calcularTrimestre2025(r, deducoesTrimestrais[i] ?? defaultDeducoes, retencoesTrimestrais[i] ?? defaultRetencoes, equiparacao, i + 1));
}
