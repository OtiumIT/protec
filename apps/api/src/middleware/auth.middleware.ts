  ...
  try {
    ...
    if (payload.companyId === null || payload.companyId === undefined) {
      result = await query<User>(
        'SELECT id, email, name, tenant_id, role, status, must_change_password, created_at, updated_at FROM public.users WHERE id = $1 AND tenant_id IS NULL',
        [payload.userId]
      );
    } else {
      result = await query<User>(
        'SELECT id, email, name, tenant_id, role, status, must_change_password, created_at, updated_at FROM public.users WHERE id = $1 AND tenant_id = $2',
