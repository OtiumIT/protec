...
export function calcularBCC(
  ...
  outrosIsentosQueEntramBase = 0,
  outrosExcluidosArt16A = 0
): number {
...
export function calcularImpactoIncrementalBase(dados: DadosIrpfAltaRenda): ImpactoIncrementalCategoria[] {
...
export function construirMemoriaLegalExclusoes(dados: DadosIrpfAltaRenda): MemoriaLegalExclusao[] {
  const itens: MemoriaLegalExclusao[] = [];
...
export function simularOtimizacaoIsentoVsTributado(
  ...
  const adicionalIrpfmNoCenarioTributado = round2(
  ...
  return {
    ...
    observacao:
      baseCalculoAtual > LIMITE_PROGRESSIVA
...
export function classificarIsentosArt16A(
  itens: Array<{ codigo?: string; descricao?: string; nome_fonte?: string; valor?: number }>
): {
