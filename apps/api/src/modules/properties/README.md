...
- **Despesas dedutíveis (PF)**: IPTU, condomínio (quando pago pelo locador), taxa imobiliária, taxa plataforma — exclusões da receita de aluguel (Lei n. 7.739/1989, art. 14; art. 42 do RIR/2018)
...
- **Reforma PF — contribuinte IBS/CBS**: pelo regulamento da LC 214/2025, a PF só é contribuinte quando atende cumulativamente: (i) mais de 3 imóveis (4 ou mais) **e** (ii) receita anual > R$ 240k (limite indexado por IPCA). O patamar de R$ 288k (240k + 20%) **não** torna a PF contribuinte por si só — exige sempre > 3 imóveis (regra de antecipação no mesmo ano-calendário). Quando a PF **não** é contribuinte, `reforma_2027_pf.imposto_total` = apenas IR (Carnê-Leão), `reforma_2027_pf.ibs_cbs_liquido` = 0. No comparativo de cenários do portal, a coluna Reforma PF exibe "—" (não se aplica) nesses casos.
...
- `opcoes_reforma.considerar_creditos_ibs_cbs`: se `false` (padrão), o cenário Reforma não aproveita crédito de IBS/CBS. Só aplica crédito quando o usuário marca que possui créditos a utilizar.
